...
def click_submit():
    ...
    if f_name == '' or l_name == '' or phone == 0 or email == '' or address == '' or room_no == 0 \
        ...
        mydb = mysql.connector.connect(host='localhost', user='SYSTEM', password='jitin', database='hotel')
        alldb = mysql.connector.connect(host='localhost', user='SYSTEM', password='jitin', database='hotel')
        allcur = alldb.cursor()
        cur = mydb.cursor()
